...
const MiroOrb = ({ isAwake, isListening, isSpeaking, onClick }: MiroOrbProps) => {
  ...
  const ringVariants: Variants = {
    ...
    awake: { opacity: 0.5, rotate: 360, transition: { rotate: { duration: 20, repeat: Infinity, ease: "linear" as const } } },
    listening: { opacity: 0.7, rotate: 360, transition: { rotate: { duration: 8, repeat: Infinity, ease: "linear" as const } } },
    speaking: { opacity: 1, rotate: 360, transition: { rotate: { duration: 4, repeat: Infinity, ease: "linear" as const } } },
  };
  ...
  return (
    <div className="relative flex items-center justify-center cursor-pointer" onClick={onClick}>
      ...
      <motion.div
        ...
        transition={{ duration: state === "speaking" ? 6 : 15, repeat: Infinity, ease: "linear" as const }}
      />
      <motion.div
      ...
      >
        <motion.div
          ...
          transition={{ duration: state === "speaking" ? 0.6 : 2, repeat: Infinity, ease: "easeInOut" as const }}
        />
      </motion.div>
    </div>
  );
